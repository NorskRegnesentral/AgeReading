...
for(m in 1:Nsim)
{
  ...
  save(simfitcomb0.year,file=paste0("simfitcomb0M6",m,".",Nmcmc,".RData"))
}
...
{
  print(m)
  load(paste0("simfitcomb0M6",m,".",Nmcmc,".RData"))              #reader dependent age errors
...
save(crpsM6,file="crpsM6.RData")
ages = matrix(rep(2+1:A,Nsim),nrow=A,byrow=T)
for(y in 1:3)
{
  pdf(paste0("Sim_M6_N_year",2020+y,".pdf"),height=5,width=10)
