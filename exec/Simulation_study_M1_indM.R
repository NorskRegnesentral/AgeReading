...
for(m in 3:Nsim)
{
  ...
  save(simfitcomb1.year,file=paste0("../save_results/simfitcomb1M1_indM",m,".",Nmcmc,".RData"))
}
...
{
  print(m)
  load(paste0("../save_results/simfitcomb1M1_indM",m,".",Nmcmc,".RData"))              #reader dependent age errors
...
for(y in 1:3)
{
  pdf(paste0("Sim_M1_N_year_indM",2020+y,".pdf"),height=5,width=10)
  matplot(t(ages),t(fM1[,2,y,1:A]),type="l",lty=1,col=2,log="y",xlab="",
          ylab="",
...
withinfM1 = matrix(nrow=3,ncol=A)
for(y in 1:3)
  for(a in 1:A)
    withinfM1[y,a] = mean(Truef[y,a] %[]% fM1[,c(1,3),y,a])
