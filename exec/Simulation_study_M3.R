...
if(SIM)
{
  for(m in 1:Nsim)
  {
    ...
    save(simfitcomb0.year,file=paste0("simfitcomb0M3",m,".",Nmcmc,".RData"))
  }
}
...
{
  print(m)
  load(paste0("../save_results/simfitcomb0M3",m,".",Nmcmc,".RData"))              #reader dependent age errors
...
save(crpsM3,file="crpsM3.RData")
ages = matrix(rep(2+1:A,Nsim),nrow=A,byrow=T)
for(y in 1:3)
{
  pdf(paste0("Sim_M3_N_year",2020+y,".pdf"),height=5,width=10)
