...
for(m in 3:Nsim)
{
  ...
  save(simfitcomb1.year,file=paste0("../save_results/simfitcomb1M6_indM",m,".",Nmcmc,".RData"))
}
...
crpsM6 = matrix(nrow=A,ncol=Nsim)
crpsM6 = array(NA,c(Y,A,Nsim))
for(m in 1:Nsim)
{
  print(m)
  load(paste0("../save_results/simfitcomb1M6_indM",m,".",Nmcmc,".RData"))              #reader dependent age errors
...
for(y in 1:3)
{
  pdf(paste0("Sim_M6_N_year_indM",2020+y,".pdf"),height=5,width=10)
  matplot(t(ages),t(fM6[,2,y,1:A]),type="l",lty=1,col=2,log="y",xlab="",ylab="",
